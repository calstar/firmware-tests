...
int run_test() {
  ...
  rslt = bmp3_init(&dev);
  if (rslt != BMP3_OK)
    while(1) fprintf(debug_uart, "bmp3_init failed\r\n");
  ...
  if (rslt != BMP3_OK)
    while(1) fprintf(debug_uart, "bmp3_set_sensor_settings failed\r\n");
  dev.settings.op_mode = BMP3_NORMAL_MODE;
  rslt = bmp3_set_op_mode(&dev);
  if (rslt != BMP3_OK)
    while(1) fprintf(debug_uart, "bmp3_set_op_mode failed\r\n");

  struct bmp3_data data;
  while(1)
    if (bmp3_get_sensor_data(BMP3_ALL, &data, &dev) == BMP3_OK)
//      fprintf(debug_uart, "pressure: %f, temperature: %f\r\n", data.pressure, data.temperature);
        fprintf(debug_uart, "Altitude: %f\r\n", (1 - pow(data.pressure / 100 / 1013.25, 0.190284)) * 145366.45);
    else
      fprintf(debug_uart, "reading failed\r\n");
